...
module.exports = {
  ...
  registerRoutes (config, server, app) {
    ...
    if (app.config.main.paths.www) {
      const staticDir = path.resolve(app.config.main.paths.root, app.config.main.paths.www)
      server.route({
        method: 'GET',
        path: path.resolve(staticDir, '{filename*}'),
        handler: {
          file: function(request) {
            return path.resolve(staticDir, request.params.filename)
          }
        }
      })
    }
