  ...
  registerRoutes (config, server, app) {
    ...
    if (app.config.main.paths.www) {
      if(Array.isArray(app.config.main.paths.www)){
        app.config.main.paths.www.forEach(item =>{
          let staticDir = path.relative(app.config.main.paths.root, item.path)
          server.route({
            method: 'GET',
            path: item.humanUrl ? item.humanUrl.concat('/{filename*}') : '/'.concat(staticDir.replace(/\\/g, '/'), '/{filename*}'),
            handler: {
